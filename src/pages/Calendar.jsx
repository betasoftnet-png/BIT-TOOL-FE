...
export default function Calendar() {
  ...
  const isDayViewLoading = false; // Query removed, use sync data
  ...
  const dayViewData = useMemo(() => {
    if (!selectedDate || !groupedData[selectedDate]) return { events: [], notes: [], reminders: [] };
    ...
    <div className="max-w-6xl mx-auto h-full flex flex-col p-4 md:p-8">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-8 gap-4">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-2 text-sm transition-colors">Manage your events, notes, and reminders</p>
        </div>
        ...
        <div className="flex items-center gap-3 w-full md:w-auto flex-wrap">
          ...
          <select 
            ...
            className="px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 outline-none transition-all text-sm font-medium bg-white dark:bg-gray-800 dark:text-white cursor-pointer"
          >
            ...
            <AnimatePresence>
              {isSearchFocused && searchQuery.length > 1 && (
                <motion.div 
                  ...
                  className="absolute top-full left-0 right-0 mt-2 bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-gray-100 dark:border-gray-700 z-50 max-h-96 overflow-y-auto"
                >
                  {isSearching ? (
                      ...
                      {searchResults.events?.length > 0 && (
                        <div>
                          ...
                          {searchResults.events.map(ev => (
                            <div key={ev.id} onClick={() => { setSelectedDate(ev.startTime.split('T')[0]); openModal(null, ev, 'event'); setIsSearchFocused(false); }} className="px-3 py-2 hover:bg-gray-50 dark:hover:bg-gray-700/50 rounded-lg cursor-pointer flex items-center gap-3 transition-colors">
                      ...
                      {searchResults.notes?.length > 0 && (
                        <div>
                          ...
                          {searchResults.notes.map(nt => (
                            <div key={nt.id} onClick={() => { setSelectedDate(nt.date.split('T')[0]); openModal(null, nt, 'note'); setIsSearchFocused(false); }} className="px-3 py-2 hover:bg-gray-50 dark:hover:bg-gray-700/50 rounded-lg cursor-pointer flex items-center gap-3 transition-colors">
                      ...
                      {searchResults.reminders?.length > 0 && (
                        <div>
                          ...
                          {searchResults.reminders.map(rm => (
                            <div key={rm.id} onClick={() => { setSelectedDate(rm.date.split('T')[0]); openModal(null, rm, 'reminder'); setIsSearchFocused(false); }} className="px-3 py-2 hover:bg-gray-50 dark:hover:bg-gray-700/50 rounded-lg cursor-pointer flex items-center gap-3 transition-colors">
          ...
          <div className="flex items-center bg-white dark:bg-gray-800 p-1.5 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 transition-colors">
            <button onClick={prevMonth} className="p-2 hover:bg-gray-50 dark:hover:bg-gray-700/50 rounded-lg text-gray-600 dark:text-gray-400 transition-colors">
              <ChevronLeft size={18} />
            </button>
            <span className="font-bold text-gray-800 dark:text-gray-100 min-w-[130px] text-center text-sm">
              {monthNames[month]} {year}
            </span>
            <button onClick={nextMonth} className="p-2 hover:bg-gray-50 dark:hover:bg-gray-700/50 rounded-lg text-gray-600 dark:text-gray-400 transition-colors">
              <ChevronRight size={18} />
            </button>
          </div>
          <button onClick={goToToday} className="px-4 py-2.5 text-sm font-bold text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/30 hover:bg-blue-100 dark:hover:bg-blue-900/50 rounded-xl transition-colors">
            Today
          </button>
        </div>
      </div>

      <div className="flex-1 bg-white dark:bg-gray-800 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-none border border-gray-100 dark:border-gray-700 flex flex-col overflow-hidden relative transition-colors">
        {isMonthLoading && (
        ...
        {/* Days Header */}
        <div className="grid grid-cols-7 border-b border-gray-100 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-900/50 transition-colors">
          {dayNames.map(day => (
            <div key={day} className="py-2 md:py-3 text-center text-[10px] md:text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider truncate">
              {day}
            </div>
          ))}
        </div>
        ...
        {/* Calendar Grid */}
        <div className="flex-1 grid grid-cols-7 auto-rows-[minmax(70px,1fr)] md:auto-rows-[minmax(120px,1fr)] gap-px bg-gray-100 dark:bg-gray-900 overflow-y-auto custom-scrollbar">
          {/* Empty cells for padding */}
          {Array.from({ length: firstDay }).map((_, i) => (
            <div key={`empty-${i}`} className="bg-white/50 dark:bg-gray-800/50 p-1 md:p-2 opacity-50"></div>
          ))}
          ...
          {Array.from({ length: daysInMonth }).map((_, i) => {
            ...
            return (
              <div 
                key={day} 
                className="bg-white dark:bg-gray-800 p-1 md:p-2 flex flex-col hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors cursor-pointer group min-h-[70px] md:min-h-[120px]"
                onClick={(e) => {
                  if (e.target === e.currentTarget || e.target.tagName === 'SPAN' || e.target.tagName === 'DIV') openDayView(day);
                }}
              >
                <div className="flex justify-center md:justify-between items-center mb-1">
                  <span className={`w-6 h-6 md:w-7 md:h-7 flex items-center justify-center rounded-full text-xs md:text-sm font-bold transition-colors ${isToday ? 'bg-blue-600 text-white shadow-md shadow-blue-200 dark:shadow-none' : 'text-gray-700 dark:text-gray-300 group-hover:bg-gray-200 dark:group-hover:bg-gray-600'}`}>
                    {day}
                  </span>
                  <button 
                    onClick={(e) => { e.stopPropagation(); openModal(day); }}
                    className="hidden md:block opacity-0 lg:opacity-100 md:group-hover:opacity-100 p-1 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-all"
                  >
                    <Plus size={16} />
                  </button>
                </div>
                
                <div className="flex-1 overflow-y-auto space-y-1 mt-0.5 md:mt-1 no-scrollbar pr-0 md:pr-1">
                  {dayHolidays.map((h, i) => (
                    <div 
                      key={`hol-${i}`}
                      className={`text-[9px] md:text-xs px-1 md:px-2 py-0.5 md:py-1 rounded truncate border ${h.type === 'IN' ? 'bg-orange-50 dark:bg-orange-900/30 text-orange-700 dark:text-orange-400 border-orange-100/50 dark:border-orange-800/50' : 'bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-400 border-green-100/50 dark:border-green-800/50'}`}
                      title={h.name}
                    >
                      <span className="hidden md:inline">{h.type === 'IN' ? '🇮🇳' : '🌎'} </span>{h.name}
                    </div>
                  ))}
        ...
        {isDayViewOpen && selectedDate && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/40 backdrop-blur-sm" onClick={closeDayView}>
            <motion.div 
              ...
              className="bg-white dark:bg-gray-800 rounded-3xl p-6 w-full max-w-md shadow-2xl border border-gray-100 dark:border-gray-700 flex flex-col max-h-[85vh] transition-colors"
            >
              <div className="flex justify-between items-center mb-5 shrink-0">
                ...
                <button onClick={closeDayView} className="text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 p-1.5 rounded-full transition-colors">
                  <X size={20} />
                </button>
              </div>
              ...
              <div className="mt-5 pt-4 border-t border-gray-100 dark:border-gray-700 shrink-0">
                <button 
                  onClick={() => openModal(null)}
                  className="w-full flex items-center justify-center gap-2 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold transition-all shadow-md shadow-blue-200 dark:shadow-none"
                >
                  <Plus size={18} />
                  Add New Item
                </button>
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/40 backdrop-blur-sm">
            <motion.div 
              ...
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 20 }}
              className="bg-white dark:bg-gray-800 rounded-3xl p-6 w-full max-w-md shadow-2xl border border-gray-100 dark:border-gray-700 transition-colors"
            >
              <div className="flex justify-between items-center mb-5">
                <h2 className="text-xl font-black text-gray-900 dark:text-white">
                  {editingItem ? `Edit ${editingItem.type.charAt(0).toUpperCase() + editingItem.type.slice(1)}` : 'Create New Item'}
                </h2>
                <button onClick={closeModal} className="text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 p-1.5 rounded-full transition-colors">
                  <X size={20} />
                </button>
              </div>
                ...
                <div className="flex bg-gray-100 dark:bg-gray-900 p-1 rounded-xl mb-6">
                  {['event', 'note', 'reminder'].map(tab => (
                    <button
                      ...
                      className={`flex-1 py-2 text-sm font-bold rounded-lg capitalize transition-all ${activeTab === tab ? 'bg-white dark:bg-gray-700 text-blue-600 dark:text-blue-400 shadow-sm' : 'text-gray-500 hover:text-gray-700 dark:hover:text-gray-300'}`}
                    >
                      {tab}
                    </button>
                  ))}
              ...
              <form onSubmit={handleSave} className="space-y-4">
                <div>
                  ...
                  <input 
                    ...
                    className="w-full px-4 py-2.5 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl focus:bg-white dark:focus:bg-gray-800 focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none transition-all font-medium text-gray-900 dark:text-white text-sm"
                ...
                {activeTab === 'reminder' && (
                  <div>
                    <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1.5">Reminder Time</label>
                    <input 
                      type="time" 
                      required
                      value={startTime}
                ...
                <div>
                  ...
                  {!isCreatingCategory ? (
                    <select
                      ...
                      className="w-full px-4 py-2.5 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl focus:bg-white dark:focus:bg-gray-800 focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none transition-all font-medium text-gray-900 dark:text-white text-sm appearance-none"
                    >
                      <option value="">No Category</option>
                      {categories.map(c => (
                        <option key={c.id} value={c.id}>{c.name}</option>
                      ))}
                      <option value="NEW" className="font-bold text-blue-600 dark:text-blue-400">+ Add New Category</option>
                    </select>
                  ) : (
                    <div className="flex items-center gap-2">
                      ...
                      <input
                        ...
                        className="flex-1 px-3 py-2 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl focus:bg-white dark:focus:bg-gray-800 focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none font-medium text-sm text-gray-900 dark:text-white"
                      />
                      <button 
                        type="button" 
                      ...
                      <button 
                        ...
                        className="px-2 py-2 text-gray-400 hover:text-gray-600 bg-gray-100 dark:bg-gray-700 dark:hover:bg-gray-600 rounded-xl"
                      >
                        <X size={16} />
                      </button>
                    </div>
                ...
                <div>
                  ...
                  <textarea 
                    ...
                    className="w-full px-4 py-3 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl focus:bg-white dark:focus:bg-gray-800 focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none transition-all resize-none font-medium text-gray-900 dark:text-white text-sm"
                ...
                <div className="flex items-center gap-3 pt-6 border-t border-gray-100 dark:border-gray-700">
                  {editingItem && (
                    <button 
                      ...
                      className="px-5 py-2.5 text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/30 hover:bg-red-100 dark:hover:bg-red-900/50 font-bold rounded-xl transition-colors text-sm"
                    >
                      Delete
                    </button>
                  )}
                  ...
                  <button 
                    type="button" 
                    onClick={closeModal}
                    className="px-5 py-2.5 text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 font-bold rounded-xl transition-colors text-sm"
                  >
                    Cancel
                  </button>
                  <button 
                    type="submit" 
                    className="px-6 py-2.5 bg-blue-600 text-white font-bold rounded-xl hover:bg-blue-700 transition-colors shadow-sm shadow-blue-200 dark:shadow-none text-sm flex items-center gap-2"
                  >
                    Save {activeTab}
                  </button>
                </div>
